...
- `AI 判断`：填写"详细需求"，提交后会弹出独立进度弹窗，后台异步生成分析标准。
- `关键词判断`：填写关键词规则，任务会直接创建，不经过 AI 生成流程。
- `区域筛选`：已改为省 / 市 / 区三级选择器，数据基于闲鱼页面抓取快照内置。

### 通知推送规则

只有当商品被判定为"推荐"时才会发送通知，具体规则取决于任务的 `decision_mode`：

| 决策模式 | AI 开关状态 | 通知触发条件 |
|----------|-------------|--------------|
| `关键词判断` | 任意 | 商品标题/描述中**命中配置的关键词**时通知 |
| `AI 判断` | 关闭 | 跳过 AI 分析，**所有商品都通知** |
| `AI 判断` | 开启 | AI 分析返回 `is_recommended=true` 时通知 |

**配置建议：**
- 如果想监控特定商品（如"海底捞优惠券"），使用 `关键词判断` 模式，配置精准关键词
- 如果想监控某个卖家的所有商品，使用 `AI 判断` 模式并**关闭 AI 开关**，实现无差别通知
- 如果需要复杂的筛选逻辑（如"只要非商家、价格低于 100 元"），使用 `AI 判断` 模式并开启 AI，在 AI Prompt 中描述规则
